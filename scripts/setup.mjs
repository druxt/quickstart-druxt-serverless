 ...
 *   -> webserver), which also writes BASE_URL to .env. No OAuth here -
 *   this repo has no simple_oauth/consumers, so there's no client ID
 *   step to wait on.
 ...
 * makes `npx giget gh:druxt/quickstart-druxt-serverless my-site --install` install
...
async function doSetup({ splash }) {
  ...
  console.log('Setting up the Druxt quickstart (serverless) (Drupal backend + Nuxt frontend)...')
  console.log('')
