 ...
 *
 * The file is written by drupal/.devtools (start writes BASE_URL) - it
 * is not parsed with a dependency, just enough of the dotenv format for
 * KEY=VALUE lines.
 */
...
export function printCommands() {
  ...
  console.log('    npm run generate   Build full static output (nuxt/dist) - needs the backend up')
...
export async function ensureBackend() {
  ...
  if (!up) {
    ...
    console.error('Log: /tmp/quickstart-serverless-drupal-php-server.log')
    process.exit(1)
  }
